<script lang="ts">
  ...
  import type { PortalRole } from '$lib/portalPaths';

  type ProfileData = NonNullable<GetMembershipsResponse['data']>;

  let { data, portalRole = 'student' }: { data: { profileData: ProfileData | null }; portalRole?: PortalRole } = $props();

  const portalLabel = $derived(
    portalRole === 'tutor' ? 'Teacher' : portalRole === 'institute' ? 'Institute' : 'Student'
  );
...
<div class="mx-auto w-full max-w-2xl min-w-0 text-[var(--page-text)]">
  <div class="mb-6">
    ...
    <p class="mt-1 text-sm text-[var(--page-text-muted)]">{portalLabel} portal · account & profile</p>
  </div>
