...
<div class="relative mx-auto max-w-5xl px-4 pb-28 pt-2 sm:px-5 sm:pb-10">
	<header class="mb-10 text-center sm:mb-12">
		...
		{#if trialUsed}
			<p
				class="mx-auto mt-4 max-w-2xl rounded-xl border border-[var(--sh-exam-card-border)] bg-[color-mix(in_srgb,var(--sh-exam-card-arrow-bg)_40%,transparent)] px-4 py-3 text-xs text-[var(--sh-ai-sub)] sm:text-sm"
			>
				You’ve already used your free trial on your default profile. Trial plans are hidden —
				choose a paid option to continue.
			</p>
		{/if}
	</header>
