    ...
    try {
      if (editingBatchId) {
        ...
        if (Object.keys(patch).length > 0) {
          const updateRes = await updateBatchDetails(editingBatchId, patch, fetch, { token: $authStore.token });
      ...
      const res = await createBatch({
        ...
        endTime,
        ...(parsedCapacity != null ? { maxCapacity: parsedCapacity } : {})
      }, fetch, { token: $authStore.token });
      if (!res.success) {
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {#each Array(6) as _}
          <div class="skel-card rounded-2xl p-5">
            <Skeleton width="w-12" height="h-12" rounded="rounded-xl" className="mb-4" strong />
